...
const handler = async (m, { conn, usedPrefix, command, groupMetadata, participants, isPrems }) => {
    ...
    if (new Date - global.db.data.users[m.sender].monster_hunt < 3600000) {
        return m.reply(`Debes esperar un poco más para volver a cazar monstruos. Tiempo restante: ${msToTime(date - new Date())}`);
    }

    const monsters = ['Goblin', 'Ogro', 'Esqueleto', 'Dragón', 'Demonio'];

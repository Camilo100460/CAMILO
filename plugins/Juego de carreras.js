...
const handler = async (m, {conn, text, args, usedPrefix, command}) => {
  ...
  try {
    ...
    const prep = generateWAMessageFromContent(m.chat, {extendedTextMessage: {text: texto, contextInfo: {externalAdReply: {title: '𝙈𝙖𝙮-𝘽𝙤𝙩', body: null, thumbnail: imagen1, sourceUrl: 'https://github.com/BrunoSobrino/TheMystic-Bot-MD'}, mentionedJid: [m.sender]}}}, aa);
    await conn.relayMessage(m.chat, prep.message, {messageId: prep.key.id, mentions: [m.sender]});
    const dataFn = await conn.getFile(`${CFROSAPI}/api/tiktokv2?url=${args[0]}`);
    const desc1n = `_*DESCARGAS - TIKTOK*_  🎧\n\n*[ 💡 ] Responde con el comando al vídeo.* _${usedPrefix}tomp3_ *para convertirlo en audio.*`;
